			...
			updateCart(index, num) {
				...
				this.$axios.put(url).then(response => {
					if (response.data.result == 1) {
						...
						alert('向购物车中更新食品失败！'+response.data.result);
					}
				}).catch(error => {
					console.error(error);
				});
			...
			removeCart(index) {
				...
				this.$axios.delete(url).then(response => {
					if (response.data.result == 1) {
						...
						alert('从购物车中删除食品失败！'+response.data.result);
					}
				}).catch(error => {
					console.error(error);
				});
		...
		computed: {
			...
			totalPrice() {
				...
				for (let item of this.foodArr) {
					total = total.plus(new Decimal(item.foodPrice).times(item.quantity));
				}
				return total;
			},
			...
			//结算总价格
			totalSettle() {
			/* 	const totalPrice = this.totalPrice !== undefined ? new Decimal(this.totalPrice) : new Decimal(0);
				const deliveryPrice = this.business.deliveryPrice !== undefined ? new Decimal(this.business.deliveryPrice) : new Decimal(
					0);
				const total = totalPrice.plus(deliveryPrice);
