	...
	<div class="wrapper">
		...
		<div class="order-deliveryfee">
		  ...
		  <p>
			...
			<input type="checkbox" v-model="isCheck"  @click="choice(this.open)"/>
	...
	export default {
		...
		computed: {
			totalPrice() {
				...
				for (let cartItem of this.cartArr) {
					totalPrice1 += cartItem.food.foodPrice * cartItem.quantity;
				}
				totalPrice1 += this.business.deliveryPrice ;
		...
		methods: {
			...
			toPayment() {
				...
				//创建订单
				// +'/'+this.open
				let url = 'OrdersController/createOrders/'+this.user.userId+'/'+this.businessId+'/'+this.deliveryaddress.daId+'/'+this.totalPrice+'/'+this.open;
